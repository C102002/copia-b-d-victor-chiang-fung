...
public class AgregarRol extends JDialog {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AgregarRol frame = new AgregarRol((JFrame)null);
	 ...
	 */
	public AgregarRol(JFrame ventanaPadre) {
		super(ventanaPadre);
		setModal(true);
		...
		lblAgregarRol.setFont(new Font("Tahoma", Font.PLAIN, 14));
		lblAgregarRol.setBounds(169, 11, 123, 17);
		contentPane.add(lblAgregarRol);
		...
		btnAgregar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!manejoDeString.esVacio(txtField_Nombre.getText()) && !manejoDeString.esVacio(txtField_Descripcion.getText())) 
				{
					...
					{
						try {
							sqlController.UpdateQuery("INSERT INTO vah_roles(nombrerol, descripcion) "
													+  "VALUES ('" + txtField_Nombre.getText() + "', '" + txtField_Descripcion.getText() + "');");
							JOptionPane.showMessageDialog(thisInstance, "Se registró el rol exitosamente", "Registro Exitoso", JOptionPane.INFORMATION_MESSAGE);
	...
	public AgregarRol(JDialog ventanaPadre) 
	{
		super(ventanaPadre);
		setModal(true);
		setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);
